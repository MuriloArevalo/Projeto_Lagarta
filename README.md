# Calendario de eventos FCI 
Uma explicação inicial sobre o processo de ideação.

O projeto se dá sobre uma necessidade dos alunos da Faculdade de Computação e Informática da Universidade Presbiteriana Mackenzie de estarem amplamente conectados com todos os eventos que a faculdade promove. Dessa forma, tivemos a ideia de montar um site que mostre todos os eventos que acontecem no nosso prédio. O site tem como intuito informar os alunos da FCI sobre cursos, palestras, eventos e também todo tipo de atividade que conte para horas complementares. Pegaremos as informações dos portais oficiais da UPM e FCI, assim como as redes sociais.

A proposta se deu após longos debates sobre qual necessidade iríamos tentar solucionar e optamos sobre fazer algo que nos introduzisse mais ao sistema acadêmico da faculdade.  
Pegaremos as informações dos portais oficiais da UPM e FCI, assim como as redes sociais.


Imagens do protótipo inicial desenvolvido.
<img width="1845" height="825" alt="image" src="https://github.com/user-attachments/assets/ed3d2ee6-7434-415f-8848-bbd3006c022e" />
<img width="1846" height="827" alt="image" src="https://github.com/user-attachments/assets/2a95b979-3020-4a34-b348-8beeddc67616" />




Uma explicação sobre o caráter extensionista.

O projeto possui caráter extensionista porque nasce de uma necessidade real dos alunos da Faculdade de Computação e Informática da Universidade Presbiteriana Mackenzie: o acesso organizado às informações sobre eventos, cursos e atividades complementares.

Ao desenvolver um site que centraliza dados dos portais oficiais e redes sociais institucionais, a iniciativa beneficia diretamente a comunidade acadêmica, promovendo maior participação, integração e democratização da informação dentro da universidade.





Nome e RA dos integrantes.
Gustavo Biassio 10738330
Murilo Arevalo 10743851
Victor Feitosa 10755205
